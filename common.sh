...
info_run() {
  ...
  # The timer id prevents collisions, e.g. `info_run sudo ...` always has sudo
  # parsed as the command, which frequently happens multiple times in a script.
  local timer_id
  timer_id=$(uuidgen)
  emit_build_metric "start-timer" "timer.${cmd}" "${timer_id}"

  local start_time
  start_time="$(date +%s)"
  "$@" || rc="$?"
  print_time_elapsed "" "${cmd}"

  emit_build_metric "stop-timer" "timer.${cmd}" "${timer_id}"
  if [[ "${rc}" != "0" ]]; then
    die "Command (${cmd}) failed (rc=${rc})"
  fi
...
emit_build_metric() {
  # Emit a build metric, see emit_metric for details.
  local emit_metric="/mnt/host/source/chromite/bin/emit_metric"
  if [[ -n "${CROS_METRICS_DIR}" ]] && [[ -e "${emit_metric}" ]] ; then
    local operation=$1
    local metric=$2
    local key=${3:-${metric}}
